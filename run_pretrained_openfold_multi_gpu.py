...
def main(args):
    # Initialize MPI environment if requested
    if args.mpi_plugin:
        ...
        if torch.cuda.device_count() > 1:
            ...
            args.model_device = f"cuda:{local_rank}"
    ...
    for model, output_directory in model_generator:
        ...
        if args.mpi_plugin and torch.cuda.device_count() > 1:
        ...
        for (tag, tags), seqs in sorted_targets:
            ...
            # Move features to appropriate device
            processed_feature_dict = {
                k: torch.as_tensor(v, device=args.model_device)
                for k, v in processed_feature_dict.items()
            }

            # Run model with distributed processing
            out = run_model(model, processed_feature_dict, tag, args.output_dir)

            # Only save outputs on rank 0 when using DDP
            is_main_process = not args.mpi_plugin or dist.get_rank() == 0
            if is_main_process:
...
def add_mpi_args(parser):
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Whether to use MPI for distributed processing"
    )
